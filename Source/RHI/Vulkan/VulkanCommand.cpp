...
VulkanCommandBuffer::VulkanCommandBuffer(VulkanDevice* Device, VulkanCommandBufferPool* Pool, ECommandBufferLevel Level, bool8_t UseForTransfer)
	: VkHWObject(Device, Level)
	, m_Pool(Pool)
	, m_Barrier(std::move(std::make_unique<VulkanPipelineBarrier>(this)))
	, m_Semaphore(std::move(std::make_unique<VulkanSemaphore>(Device)))
	, m_UseForTransfer(UseForTransfer)
{
	/**************************************************************************************************************************************************
...
void VulkanCommandBuffer::BeginDebugMarker(const char8_t* Name, const Color& MarkerColor)
{
	...
	if (m_Device->EnabledExtensions().DebugMarker)
	{
		VkDebugMarkerMarkerInfoEXT MarkerInfo
		{
			...
			nullptr,
			Name,
			{ MarkerColor.x, MarkerColor.y, MarkerColor.z, MarkerColor.w }
		};
...
void VulkanCommandBuffer::Draw(uint32_t vertexCount, uint32_t firstVertex)
{
	...
	vkCmdDraw(m_Handle, vertexCount, 1u, firstVertex, 0u);
}
...
void VulkanCommandBuffer::DrawIndirect(IBuffer* IndirectBuffer, size_t Offset, uint32_t DrawCount, uint32_t Stride)
{
	assert(m_State == EState::Recording && IndirectBuffer && m_InsideRenderPass);

	vkCmdDrawIndirect(m_Handle, static_cast<VulkanBuffer*>(IndirectBuffer)->Get(), Offset, DrawCount, Stride);
}
...
void VulkanCommandBuffer::DrawIndexed(uint32_t IndexCount, uint32_t FirstIndex, int32_t VertexOffset)
{
	...
	vkCmdDrawIndexed(m_Handle, IndexCount, 1, FirstIndex, VertexOffset, 0u);
}
...
void VulkanCommandBuffer::DrawIndexedIndirect(IBuffer* IndirectBuffer, size_t Offset, uint32_t DrawCount, uint32_t Stride)
{
	assert(m_State == EState::Recording && IndirectBuffer && m_InsideRenderPass);

	vkCmdDrawIndexedIndirect(m_Handle, static_cast<VulkanBuffer*>(IndirectBuffer)->Get(), Offset, DrawCount, Stride);
}
...
void VulkanCommandBuffer::Dispatch(uint32_t GroupX, uint32_t GroupY, uint32_t GroupZ)
{
	...
	vkCmdDispatch(m_Handle, GroupX, GroupY, GroupZ);
}

void VulkanCommandBuffer::DispatchIndirect(IBuffer* IndirectBuffer, size_t Offset)
{
	assert(m_State == EState::Recording && IndirectBuffer && m_InsideRenderPass);

	vkCmdDispatchIndirect(m_Handle, static_cast<VulkanBuffer*>(IndirectBuffer)->Get(), Offset);
}

void VulkanCommandBuffer::CopyBufferToImage(IImage* DstImage, const void* SrcBuffer, uint32_t BufferSize, const ImageSubresourceRange& SubresourceRange)
{
#if true
	if (m_InsideRenderPass)
	{
...
void VulkanCommandBuffer::CopyBuffer(IBuffer* DstBuffer, const void* Data, size_t DataSize, size_t SrcOffset, size_t DstOffset)
{
...
void VulkanCommandBuffer::SetViewport(const Viewport& DstViewport)
{
	std::vector<VkViewport> Viewports
	{
		VkViewport
		{
			DstViewport.LeftTop.x,
			m_Device->EnabledExtensions().Maintenance1 ? DstViewport.Extent.y : DstViewport.LeftTop.y,
			DstViewport.Extent.x,
			m_Device->EnabledExtensions().Maintenance1 ? -DstViewport.Extent.y : DstViewport.Extent.y,
			DstViewport.Z.x,
			DstViewport.Z.y
		}
	};
...
void VulkanCommandBuffer::SetScissorRect(const ScissorRect& DstScissorRect)
{
	std::vector<VkRect2D> ScissorRects
	{
		VkRect2D
		{
			VkOffset2D
			{
				static_cast<int32_t>(DstScissorRect.LeftTop.x),
				static_cast<int32_t>(DstScissorRect.LeftTop.y)
			},
			VkExtent2D
			{
				static_cast<uint32_t>(DstScissorRect.Extent.x),
				static_cast<uint32_t>(DstScissorRect.Extent.y)
			}
		}
	};
...
void VulkanCommandBuffer::PushConstants(EShaderStage Stage, IBuffer* ConstantsBuffer, const void* Data, size_t Size, size_t Offset)
{
...
void VulkanCommandBuffer::SetGraphicsPipeline(IPipeline* DstPipeline)
{
...
void VulkanCommandBuffer::ClearColorImage(IImage* DstImage, const Color& ClearColor)
{
#if true
	if (m_InsideRenderPass)
	{
...
void VulkanCommandBuffer::ClearDepthStencilImage(IImage* DstImage, bool8_t ClearDepth, bool8_t ClearStencil, float32_t Depth, uint8_t Stencil)
{
#if true
	if (m_InsideRenderPass)
	{
...
void VulkanCommandBuffer::WaitCommand(ICommandBuffer* CommandToWait)
{
