...
void LogEnabledLayerAndExtensions(const VulkanLayerArray& Layers, const VulkanExtensionArray& Extensions, const char* Category)
{
	for (const auto &Layer : Layers)
	{
		if (Layer->IsEnabled())
		{
			LOG_INFO(LogVulkan, "Enable {} layer: \"{}\", spec @{}", Category, Layer->GetName(), Layer->GetSpecVersion());
		}
		else
		{
			if (Layer->IsNeeded())
			{
				LOG_ERROR(LogVulkan, "Requested {} layer \"{}\" is not supported.", Category, Layer->GetName());
			}
		}
	}
	...
	for (const auto& Extension : Extensions)
	{
		...
		{
			LOG_INFO(LogVulkan, "Enable {} extension: \"{}\", spec @{}", Category, Extension->GetName(), Extension->GetSpecVersion());
		}
		else
		{
			if (Extension->IsNeeded())
			{
				LOG_ERROR(LogVulkan, "Requested {} layer \"{}\" is not supported.", Category, Extension->GetName());
			}
		}
	}
}
