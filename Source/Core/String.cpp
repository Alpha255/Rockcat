...
namespace String
{
	void ToLower(std::string& Str)
	{
		std::transform(Str.begin(), Str.end(), Str.begin(),
			[](char8_t C)
		{
			return static_cast<char8_t>(std::tolower(static_cast<int32_t>(C)));
		});
	}
	...
	void ToUpper(std::string& Str)
	{
		std::transform(Str.begin(), Str.end(), Str.begin(),
			[](char8_t C)
		{
			return static_cast<char8_t>(std::toupper(static_cast<int32_t>(C)));
		});
	}
	...
	std::string Format(const char8_t* Fmt, ...)
	{
		if (Fmt)
		{
			...
			std::unique_ptr<char8_t[]> Ret = std::make_unique<char8_t[]>(Size);
	...
	std::string ToMultiByte(const std::wstring& Str)
	{
		...
		std::vector<char8_t> Temp(Length + 1u);
