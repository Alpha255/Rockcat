	...
	: Serializeable(IAsset::CatPath(ASSET_PATH_SCENES, Path))
{
	if (!SceneNode::IDPool)
	{
		SceneNode::IDPool = std::make_unique<IDPoolThreadSafe<SceneNode::ObjectID>>();
	}
}
...
void Scene::OnLoadCompletion()
{
	auto PreLoad = [](
		auto& Src, 
		std::unordered_map<SceneNode::ObjectID, std::shared_ptr<SceneNode>>& Dst) {
	...
	if (m_RootID != SceneNode::InvalidID)
	{
...
bool8_t Scene::AddObject(const std::shared_ptr<SceneNode>& Object, const std::shared_ptr<SceneNode>& Parent)
{
	if (Object)
	{
		assert(Object->ID() != SceneNode::InvalidID);
...
void Scene::SetRoot(std::shared_ptr<SceneNode>& Root)
{
	if (Root)
	{
		...
		{
			assert(Root->ID() != SceneNode::InvalidID);
