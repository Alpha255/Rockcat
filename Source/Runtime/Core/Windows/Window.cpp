...
#include "Runtime/Application/Resource.h"
...
static ::LRESULT MessageProc(::HWND HWnd, uint32_t Message, ::WPARAM WParam, ::LPARAM LParam)
{
	if (Message == WM_CREATE)
	{
		...
		assert(CreateStruct);
		VERIFY_PLATFORM(::SetWindowLongPtrA(HWnd, 0, reinterpret_cast<LONG_PTR>(CreateStruct->lpCreateParams)) == 0);
	}
...
Window::Window(const WindowDesc& Desc, IInputHandler* InputHandler)
	...
	, m_Width(std::max<uint32_t>(Desc.Width, Desc.MinWidth))
	, m_Height(std::max<uint32_t>(Desc.Height, Desc.MinHeight))
	, m_InputHandler(InputHandler)
{
	::HINSTANCE HInstance = reinterpret_cast<::HINSTANCE>(Platform::GetCurrentModuleHandle());
	assert(HInstance);

	::HICON Icon = ::LoadIcon(HInstance, MAKEINTRESOURCE(ICON_NVIDIA));
	VERIFY_PLATFORM(Icon);
	...
	VERIFY_PLATFORM(::RegisterClassExA(&WndClassEx) != 0);
	...
	VERIFY_PLATFORM(::AdjustWindowRect(&Rect, WS_OVERLAPPEDWINDOW, false) != 0);
	...
	VERIFY_PLATFORM(::UpdateWindow(Handle) != 0);
...
void Window::UpdateSize(bool8_t Signal)
{
	...
	{
		::RECT Rect;
		VERIFY_PLATFORM(::GetClientRect(reinterpret_cast<::HWND>(m_Handle), &Rect) != 0);
