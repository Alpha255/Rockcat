...
protected: \
	using ShaderClass = ShaderType; \
private: \
	struct FirstVariableID{}; \
	typedef void* FuncPtr; \
	typedef FuncPtr(*RegisterShaderVariableFunc)(FirstVariableID, ShaderClass&); \
	static FuncPtr RegisterShaderVariable(FirstVariableID, ShaderClass&) { return nullptr; } \
...
#define DECLARE_SHADER_VARIABLE(Type, Name, RHIType, Binding, SetterGetter) \
	...
	private: \
		...
		static FuncPtr RegisterShaderVariable(NextVariableID_##Name, ShaderClass& Shader) \
		{ \
			ShaderVariable Variable{ ERHIResourceType::Type, Binding, offsetof(ShaderClass, Name), sizeof(Type) }; \
			Variable.Set = [&Shader](const ShaderVariable::Variant& Value) { Shader.##Name=std::get<Type>(Value);}; \
			Variable.Get = [&Shader](){ return ShaderVariable::Variant(Shader.##Name); }; \
			Shader.RegisterVariable(#Name, std::move(Variable)); \
			FuncPtr(*NextShaderVariableRegisterFunc)(VariableID##Name, ShaderClass&); \
			NextShaderVariableRegisterFunc = RegisterShaderVariable; \
			return reinterpret_cast<FuncPtr>(NextShaderVariableRegisterFunc); \
		} \
...
#define DECLARE_SHADER_VARIABLES_END \
		LastVariableID; \
	public: \
		static void RegisterShaderVariables(ShaderClass& Shader) { \
			FuncPtr(*FinalRegisterShaderVariableFunc)(LastVariableID, ShaderClass& Shader); \
			...
			do \
			{ \
				Func = reinterpret_cast<RegisterShaderVariableFunc>(Func)(FirstVariableID(), Shader); \
			} while(Func); }

#define DECLARE_SV_UNIFORM_BUFFER_BEGIN(Binding) \
	VariableIDUniformBuffer; \
	private: \
...
#define DECLARE_SV_SETER_GETTER_DEFAULT(Type, Name) \
	...
	template<class T> inline ShaderVariables& Set##Name(T&& Value) { Name = std::forward<T>(Value); return *this; } \

#define DECLARE_SV_SETER_GETTER_IMAGE_ASSET(Type, Name) \
	inline const Type& Get##Name() const { return Name; } \
	template<class T> \
	void Set##Name(T&& Path) { Name = std::make_shared<ImageAsset>(std::forward<T>(Path)); }
...
#define DECLARE_SV_TEXTURE_1D(Name, Binding) DECLARE_SHADER_VARIABLE(std::shared_ptr<ImageAsset>, Name, SampledImage, Binding, DECLARE_SV_SETER_GETTER_IMAGE_ASSET(std::shared_ptr<ImageAsset>, Name))
#define DECLARE_SV_TEXTURE_ARRAY_1D(Name, Binding) DECLARE_SHADER_VARIABLE(std::shared_ptr<ImageAsset>, Name, SampledImage, Binding, DECLARE_SV_SETER_GETTER_IMAGE_ASSET(std::shared_ptr<ImageAsset>, Name))

#define DECLARE_SV_TEXTURE_2D(Name, Binding) DECLARE_SHADER_VARIABLE(std::shared_ptr<ImageAsset>, Name, SampledImage, Binding, DECLARE_SV_SETER_GETTER_IMAGE_ASSET(std::shared_ptr<ImageAsset>, Name))
#define DECLARE_SV_TEXTURE_ARRAY_2D(Name, Binding) DECLARE_SHADER_VARIABLE(std::shared_ptr<ImageAsset>, Name, SampledImage, Binding, DECLARE_SV_SETER_GETTER_IMAGE_ASSET(std::shared_ptr<ImageAsset>, Name))

#define DECLARE_SV_TEXTURE_CUBE(Name, Binding) DECLARE_SHADER_VARIABLE(std::shared_ptr<ImageAsset>, Name, SampledImage, Binding, DECLARE_SV_SETER_GETTER_IMAGE_ASSET(std::shared_ptr<ImageAsset>, Name))
#define DECLARE_SV_TEXTURE_ARRAY_CUBE(Name, Binding) DECLARE_SHADER_VARIABLE(std::shared_ptr<ImageAsset>, Name, SampledImage, Binding, DECLARE_SV_SETER_GETTER_IMAGE_ASSET(std::shared_ptr<ImageAsset>, Name))

#define DECLARE_SV_TEXTURE_3D(Name, Binding) DECLARE_SHADER_VARIABLE(std::shared_ptr<ImageAsset>, Name, SampledImage, Binding, DECLARE_SV_SETER_GETTER_IMAGE_ASSET(std::shared_ptr<ImageAsset>, Name))
...
#define DEFINITION_SHADER_VARIABLES_GENERIC_VS \
	DECLARE_SHADER_VARIABLES_BEGIN(GlobalShaders::GenericVS) \
...
#define DEFINITION_SHADER_VARIABLES_UNLIT \
	DECLARE_SHADER_VARIABLES_BEGIN(GlobalShaders::UnlitFS) \
		DECLARE_SV_UNIFORM_BUFFER_BEGIN(1) \
		DECLARE_SV_UNIFORM_BUFFER_END \
		DECLARE_SV_TEXTURE_2D(DiffuseMap, 2) \
	DECLARE_SHADER_VARIABLES_END \

#define DEFINITINO_SHADER_VARIABLES_LIT \
	DECLARE_SHADER_VARIABLES_BEGIN(GlobalShaders::LitFS) \
		DECLARE_SV_UNIFORM_BUFFER_BEGIN(1) \
		DECLARE_SV_UNIFORM_BUFFER_END \
